...
export default function OrderCompanyTab() {
  ...
  return (
    <div className="space-y-3">
      <h3 className="text-sm font-semibold text-zinc-100">발주회사</h3>

      {/* 추가 폼 */}
      <div className="bg-zinc-800/50 border border-zinc-700/60 rounded-xl p-4 space-y-3">
        <div>
          <label className="block text-xs font-medium text-zinc-400 mb-1.5">발주회사 이름</label>
      ...
      {/* 목록 */}
      {(list.data ?? []).length === 0 ? (
        <p className="text-sm text-zinc-600 px-1">발주회사를 추가하세요.</p>
      ) : (
...
function CompanyRow({ item, onSave, onDelete }: {
  ...
  if (edit) {
    return (
      <div className="bg-zinc-800 border border-indigo-500/50 rounded-xl p-3 space-y-2">
        <input
          className={inputCls}
          value={name}
        ...
        <div className="flex gap-2">
          <button
            className="flex-1 bg-indigo-600 hover:bg-indigo-500 text-white rounded-lg py-2 text-xs font-medium transition-colors"
            onClick={async () => {
              try { await onSave(name.trim(), imageUrl); setEdit(false); show("수정됨"); }
              catch (e) { show((e as Error).message ?? "오류", "error"); }
            }}>저장</button>
          <button
            className="px-3 border border-zinc-700 hover:border-zinc-600 text-zinc-400 hover:text-zinc-100 rounded-lg text-xs transition-colors"
  ...
  return (
    <div className="relative group bg-zinc-900 border border-zinc-800 hover:border-zinc-700 rounded-xl p-3 flex flex-col items-center gap-2 transition-colors">
      {/* 로고 */}
      {item.image_url ? (
        <img src={item.image_url} alt={item.name} className="w-full aspect-square object-cover rounded-lg" />
      ) : (
        <div className="w-full aspect-square rounded-lg bg-zinc-800 border border-zinc-700 flex items-center justify-center">
      ...
      {/* 이름 */}
      <p className="text-sm font-medium text-zinc-200 text-center leading-tight">{item.name}</p>
      {/* 수정/삭제 버튼 — hover 시 표시 */}
      <div className="absolute top-2 right-2 flex gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
        <button
          className="p-1 bg-zinc-800 hover:bg-zinc-700 rounded text-zinc-400 hover:text-zinc-100 transition-colors"
        ...
        <button
          className="p-1 bg-zinc-800 hover:bg-red-900/50 rounded text-zinc-400 hover:text-red-400 transition-colors"
          onClick={async () => {
